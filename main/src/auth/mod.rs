...
pub fn handle_index(
    ...
    if in_users {
        let fut = req.concat2()
            .from_err()
            .and_then(|_| Ok(super::statiq::handle_html("index.html".into())));
        Box::new(fut)
    } else {
        let fut = req.concat2()
            .from_err()
...
pub fn handle_login_post(
    (req, form): (HttpRequest<(WsChatSessionState, ::db::State, ::graphql::AppState)>,
                  Form<Login>)
) -> Box<Future<Item = HttpResponse, Error = Error>> {
    let already_logged = {
        let id = &req.identity();
    ...
    if already_logged {
        ...
        req.concat2().from_err().and_then(move |_| {
            ...
            if s.len() == 0 {
                exists = {
                    ...
                    ::users::USERS.read().unwrap().contains_key(&username.clone().unwrap())
                };
                if exists {
                    ...
                    if password0.is_some() {
                        ...
                        let password = user.get(&username.clone().unwrap()).unwrap().password.clone();
                        ...
                        Some(usercolor) => {
                            ...
                            if usercolor.len() != 7 {
                                ...
                                for c in usercolor.chars() {
                                    if c == '#' {
                                        continue;
                                    } else if c as u32 >= '0' as u32 && c as u32 <= '9' as u32 {
                                        continue;
                                    } else if c as u32 >= 'a' as u32 && c as u32 <= 'f' as u32 {
                                        continue;
                                    } else if c as u32 >= 'A' as u32 && c as u32 <= 'F' as u32 {
                                        continue;
                                    } else {
            ...
            if s.len() == 0 {
                if !exists {
                    let color = form.usercolor.clone().unwrap_or("#000000".into());
                    ...
                    match req3.state()
                        .1
                        .db
                        .send(::db::CreateUser(user_inner))
                        .wait() {
                            Err(e) => { s += &format!("{:?}", e); },
                            Ok(_)  => {
                                req3.remember(username.clone().unwrap());
                                ::users::USERS.write().unwrap().insert(username.clone().unwrap(), ::users::User {
            ...
            if s.len() == 0 {
                ...
                Ok(::statiq::handle_insert_on_html(
                    "login.html".into(),
                    &hash
                ))
            }
...
pub fn handle_login_get(
    ...
    if already_logged {
        ...
        req.concat2().from_err().and_then(move |_| {
            Ok(::statiq::handle_html("login.html".into()))
        }).responder()
    }
}

pub fn handle_logout(mut req: HttpRequest<(WsChatSessionState, ::db::State, ::graphql::AppState)>) -> HttpResponse {
    req.forget();
